...
public class AdminDialog extends JDialog {
    ...
    private JPanel createItemManagementPanel() {
        ...
        JTextField itemFeeField = new JTextField(5); // 기본요금 (현재 미사용)
        JTextField itemDepositField = new JTextField(5); // 보증금 필드 추가
        JTextField itemDeadlineField = new JTextField(5); // 반납기한 필드 추가
        JButton addItemButton = new JButton("추가");
        addItemButton.setBackground(DK_BLUE); addItemButton.setForeground(Color.BLACK);
        ...
        gbc.gridx = 2; gbc.gridy = 1; addItemPanel.add(new JLabel("보증금(원):"), gbc); // 보증금 UI
        gbc.gridx = 3; gbc.gridy = 1; addItemPanel.add(itemDepositField, gbc);
        
        gbc.gridx = 0; gbc.gridy = 2; addItemPanel.add(new JLabel("반납기한(일):"), gbc); // 반납기한 UI
        gbc.gridx = 1; gbc.gridy = 2; addItemPanel.add(itemDeadlineField, gbc);
        
        gbc.gridx = 3; gbc.gridy = 2; gbc.gridwidth = 2; gbc.anchor = GridBagConstraints.EAST; addItemPanel.add(addItemButton, gbc);
        panel.add(addItemPanel, BorderLayout.NORTH);

        // 테이블 컬럼 수정
        String[] itemColumns = {"물품명", "재고", "보증금", "반납기한(일)"};
        ...
        addItemButton.addActionListener(e -> {
            try {
                ...
                double baseFee = Double.parseDouble(itemFeeField.getText());
                int deposit = Integer.parseInt(itemDepositField.getText());
                int deadline = Integer.parseInt(itemDeadlineField.getText());

                Item newItem = new Item(newItemName, stock, baseFee, deposit, deadline);
                ...
                itemDepositField.setText(""); itemDeadlineField.setText(""); // 필드 초기화
                
                refreshItemTable(); parentFrame.refreshItemComboBox();
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(this, "재고, 요금, 보증금, 기한은 숫자로 입력해야 합니다.", "입력 오류", JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    private void refreshItemTable() { 
        ...
        for (Item item : items) { 
            itemTableModel.addRow(new Object[]{
                ...
                String.format("%d원", item.getDeposit()), // 보증금 표시
                String.format("%d일", item.getReturnDeadlineDays()) // 반납기한 표시
            }); 
        } 
    }
}