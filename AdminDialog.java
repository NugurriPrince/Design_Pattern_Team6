...
class AdminDialog extends JDialog {
    public AdminDialog(MainAppFrame parent, List<User> users, List<Item> items) {
        ...
        gbcUser.gridy = 0;
        gbcUser.weightx = 0.0;
        gbcUser.fill = GridBagConstraints.NONE;
        userPanel.add(new JLabel("ID:"), gbcUser);
        gbcUser.gridy = 1;
        userPanel.add(new JLabel("이름:"), gbcUser);
        gbcUser.gridy = 2;
        userPanel.add(new JLabel("비밀번호:"), gbcUser);
        gbcUser.gridy = 3;
        userPanel.add(new JLabel("타입:"), gbcUser);
        gbcUser.gridx = 1;
        gbcUser.gridy = 0;
        gbcUser.weightx = 1.0;
        gbcUser.fill = GridBagConstraints.HORIZONTAL;
        userPanel.add(userIdField, gbcUser);
        gbcUser.gridy = 1;
        userPanel.add(userNameField, gbcUser);
        gbcUser.gridy = 2;
        userPanel.add(userPassField, gbcUser);
        gbcUser.gridy = 3;
        userPanel.add(userTypeCombo, gbcUser);
        gbcUser.gridy = 4;
        gbcUser.fill = GridBagConstraints.NONE;
        ...
        itemPanel.add(new JLabel("물품명:"), gbcItem);
        gbcItem.gridy = 1;
        itemPanel.add(new JLabel("최대재고:"), gbcItem);
        gbcItem.gridy = 2;
        itemPanel.add(new JLabel("기본요금:"), gbcItem);
        gbcItem.gridx = 1;
        gbcItem.gridy = 0;
        gbcItem.weightx = 1.0;
        gbcItem.fill = GridBagConstraints.HORIZONTAL;
        itemPanel.add(itemNameField, gbcItem);
        gbcItem.gridy = 1;
        itemPanel.add(itemStockField, gbcItem);
        gbcItem.gridy = 2;
        itemPanel.add(itemFeeField, gbcItem);
        gbcItem.gridy = 3;
        gbcItem.fill = GridBagConstraints.NONE;
        gbcItem.anchor = GridBagConstraints.EAST;
        itemPanel.add(addItemButton, gbcItem);
